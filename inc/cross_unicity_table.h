...
/*  Hash functor for hashing a pair of state pointers based on their dereferenced
    values. Based on the boost hash_combine function, adapted for two hashes. */
...
/*  Returns the true if the underlying states of the state_pairs are equal */
...
class unicity_table_base {
    protected:
        /*  If two tables intersect, store a pointer to the other table in one of
            them. This is used to avoid double destruction of states. */
        unicity_table_base* other = nullptr;
    public:
        void set_co_table(unicity_table_base* t) { other = t; }

        /*  Returns true if this pair of states exists in the table */
        virtual bool contains(const spot::state* s1, const spot::state* s2) const = 0;
        virtual bool contains(state_pair s) const = 0;

        /*  To be used when considering a potentially new state.
            If the pair does not exist yet, it is inserted in the table and returned
            If equivalent state are present in the table, destroy the arguemnt
            pair and return the existing pair */
        ...
        /*  Same as operator(), but returns {nullptr, nullptr} if the pair already
            existed */
...
/*  Class derived from the SPOT_API state_unicity_table from "spot/twa/twa.hh"
    to accept pairs of state pointers, instead of state pointers
*/
